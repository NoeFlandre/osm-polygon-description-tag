...
5. an exact four-file per-PBF upload plan and remote verification;
6. atomic publication-state update;
...
manifest, README, and stats files needed for that source. Final metadata is
uploaded separately as exactly `README.md` and `stats.json`.
